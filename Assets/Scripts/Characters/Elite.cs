...
public class Elite : AI
{
    void Start()
    {
        ...
        abilities = new Ability[] { new Hit(this, 1), new Cut(this, 1), null, new Hunker(this, 1) };
    }
    ...
    {
        if (CheckAP(abilities[1].AP_Cost) && abilities[1].InRange(nearest.transform.position) && !nearest.GetComponent<Character>().HasEffect("Bleed")) // Cut
        {
            SetAnimation("attack");
            abilities[1].Action(nearest.GetComponent<Character>());
            SetActing(true);
        } else if (CheckAP(abilities[0].AP_Cost) && abilities[0].InRange(nearest.transform.position)) // Hit
        {
            SetAnimation("attack");
